...
class Post {
    ...
    static addPostFromDB(...args) {
        ...
        return pool.query('WITH inserted as (INSERT INTO posts (post_description, user_id) VALUES ($1, $2) ) SELECT * FROM posts JOIN users ON posts.user_id = users.user_id', args).then(results => { return results.rows })
    //     WITH inserted as (
